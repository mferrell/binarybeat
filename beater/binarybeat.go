...
import (
	...
	cfg "github.com/mferrell/binarybeat/config"
	"github.com/mferrell/binarybeat/crawler"
	"github.com/mferrell/binarybeat/publisher"
	"github.com/mferrell/binarybeat/registrar"
	"github.com/mferrell/binarybeat/spooler"

)

var once = flag.Bool("once", false, "Run filebeat only once until all harvesters reach EOF")

// Binarybeat is a beater object. Contains all objects needed to run the beat
type Binarybeat struct {
	config *cfg.Config
	done   chan struct{}
}

// New creates a new Binarybeat pointer instance.
...
func (fb *Binarybeat) Run(b *beat.Beat) error {
	var err error
	config := fb.config
	...
	if *once {
		runOnce := func() {
			logp.Info("Running binarybeat once. Waiting for completion ...")
...
func (fb *Binarybeat) Stop() {
	logp.Info("Stopping binarybeat")
